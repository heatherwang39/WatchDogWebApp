...
def video_number_processing(filedir_s,user_id, bucket_name, interval, options, begin_timestamp=-1):
    file_dir = 'https://' + bucket_name + '.s3.amazonaws.com/' + filedir_s
    image_addr = []
    if S3.file_setpublic(bucket_name, filedir_s):
        vid = cv2.VideoCapture(file_dir)
        count = 0
        fps = vid.get(cv2.CAP_PROP_FPS)
        period = vid.get(cv2.CAP_PROP_FRAME_COUNT) / fps
        ...
        while True:
            ...
            if count % interval_frames == 0:
                ...
                S3.upload_cvimage(image, "%d_unprocessed.jpg" % count, file_directory=user_id + "/" + timestamp + "/")
            count += 1
    if S3.file_setprivate(bucket_name, filedir_s):
        return
    ...
    while True:
        ...
        if count % interval_frames == 0:
            ...
            if options == 1:
                ...
                print(
                    "At " + str(datetime.fromtimestamp(int(frame_time))) + ", there are " + str(people_num) + " people")
